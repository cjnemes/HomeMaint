  ...
  for (const providerData of sampleProviders) {
    db.getDatabase()
      .prepare(
       ...
       service_types, address_city, address_state, address_postal_code, license_number, rating,
       is_preferred, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        ...
        providerData.address_state || null,
